"""Run the Stage 3 learning-rate sweep from its YAML specifications."""
...
def make_trainer_args(
    ...
    """Translate a learning-rate-run YAML entry into trainer arguments."""
    fixed = config["fixed"]
    data = config["data"]
    lora = config["lora"]
    return {
...
def validate(config: dict[str, Any], run: dict[str, Any]) -> None:
    """Reject inconsistent learning-rate-sweep inputs before allocating a GPU."""
    if run["alpha"] != config["lora"]["alpha_per_rank"] * run["rank"]:
        raise ValueError(f"{run['name']} does not preserve alpha/rank scaling.")
    if run["rank"] != config["lora"]["rank"]:
        raise ValueError(f"{run['name']} changes the fixed rank.")
    if run["learning_rate"] == config["reference"]["learning_rate"]:
        raise ValueError(
            "Learning rate 2e-5 is a completed reference, not a new run."
        )
...
def run_one(
    ...
    """Run one learning-rate candidate or reuse a complete matching output directory."""
    validate(config, run)
    ...
    resolved = {"phase3": config, "run": run, "trainer": arguments}
...
def reference(config: dict[str, Any]) -> dict[str, Any]:
    """Load the completed rank-64 result as the sweep comparison reference."""
...
def save_comparison(config: dict[str, Any], rows: list[dict[str, Any]]) -> None:
    """Save the learning-rate table and publish it as one W&B comparison run."""
    ...
    table.sort(key=lambda item: item["learning_rate"])
...
def main() -> None:
    """Execute all Stage 3 learning-rate candidates and publish their comparison."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--config", default="configs/ablation/phase3/phase3-lr.yaml"
    )
    ...
    print(
        f"Completed Stage 3. Comparison: {config['study']['comparison_output']}"
    )
